<script>
	...
	function prepareVisaPrompt() {
		...
		if (visaInterviewType == "VISA preparation") {
			...
			visapreppromt = `Imagine you're a visa interview officer at the ${travelTo} Embassy/Consulate, and you are conducting a visa interview for a traveller who is applying for an ${visaType} visa to enter the ${travelTo} from ${travelFrom}. Please make sure to ask one question at a time. Begin the interview with a greeting and proceed with the questions.`;
		}
		visaPrompt();
		submitLoader = false;
		closePopup();
